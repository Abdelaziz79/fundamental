...
const Playground: React.FC<Props> = React.memo(
  ({ codeString = defaultCodeString, autoFrameCheckbox = false }) => {
    ...
    const memoizedNodes = useMemo(() => Util.getAllNodeTypes(), []);
    const memoizedEdges = useMemo(() => Util.getAllEdgeTypes(), []);
    ...
    const getFrameElements = useCallback(
      async ({ frame, watingTime }: { frame: any[]; watingTime: number }) => {
    ...
    const handleRun = useCallback(async () => {
      ...
      setRunning(false);
    }, [code, getFrameElements, setElements, setNodes]);
...
const ExpandComp = React.memo(
  ({ handleExpand }: { handleExpand: () => void }) => {
    return (
...
ExpandComp.displayName = "ExpandComp";
