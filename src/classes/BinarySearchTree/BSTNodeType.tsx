...
export const RedNode = ({ data }: { data: { label: string } }) => {
  return (
    <div className="hover:scale-105 ease-in-out transition-all  shadow-2xl hover:shadow-red-500/35 overflow-hidden bg-gradient-to-br from-red-200 to-red-300 border-red-400 border-2 text-gray-700 rounded-full w-20 h-20 font-semibold flex items-center justify-center cursor-pointer  duration-300 hover:from-red-300 hover:to-red-400">
      <Handle type="target" position={Position.Top} className="opacity-0" />
      <Handle
        type="source"
        position={Position.Bottom}
